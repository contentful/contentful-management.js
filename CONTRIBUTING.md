...
| Tool | Version | Notes |
|---|---|---|
| Node.js | v22 (see `.nvmrc`) | Use `nvm use` to switch automatically |
| npm | 10+ | Comes with Node.js; CI uses latest |

Additional setup for integration tests: `CONTENTFUL_INTEGRATION_TEST_CMA_TOKEN` and `CONTENTFUL_ORGANIZATION_ID` environment variables are required to run integration tests against the live CMA.

# Branch Strategy

- `master` — primary development branch; releases are cut from here automatically
- `*.x` branches (e.g., `11.x`) — maintenance branches for previous major versions (critical fixes and security patches only)
- `beta` — pre-release channel; publishes to `npm@beta` dist-tag
- `canary` — experimental pre-release channel; publishes to `npm@canary` dist-tag
- `dev` — development pre-release channel; publishes to `npm@dev` dist-tag
- Feature branches — any branch name; merged to `master` via PR

# Release Process

This project uses [semantic-release](https://github.com/semantic-release/semantic-release) for fully automated releases:

1. Merge a PR to `master` (or a release branch)
2. CI runs the full test suite (unit, integration, browser, size check, demo projects)
3. If all checks pass, `semantic-release` analyzes commit messages to determine the next version
4. A new version is published to npm, a GitHub Release is created, and documentation is deployed to GitHub Pages

**Commit types that trigger releases:**
- `feat:` or `feat!:` — minor or major release  <!-- source: package.json → release.plugins[0] -->
- `fix:` — patch release  <!-- source: package.json → release.plugins[0] -->
- `build(deps):` — patch release  <!-- source: package.json → release.plugins[0] releaseRules -->
- `chore:` — no release (skipped by semantic-release)

**Dist tags:**
- `latest` — from `master`
- `beta` — from `beta` branch (pre-release)
- `canary` — from `canary` branch (pre-release)
- `dev` — from `dev` branch (pre-release)

# Pull Requests

- PR title should follow Conventional Commits format (it becomes the squash commit message)
- Required checks: build, lint, format, unit tests, size limit, integration tests, demo project tests
...
|---|---|---|
| Build (`build.yaml`) | Push/PR to any branch | Installs deps, runs `npm run build`, caches `dist/` | <!-- source: .github/workflows/build.yaml -->
| Check (`check.yaml`) | After build | Linting, formatting, unit tests with coverage, size check | <!-- source: .github/workflows/check.yaml -->
| Test Demo Projects (`test-demo-projects.yaml`) | After check | Verifies built package works in Node.js and browser demo apps | <!-- source: .github/workflows/test-demo-projects.yaml -->
| Test Integration (`test-integration.yaml`) | After demo tests | Runs integration tests against live CMA | <!-- source: .github/workflows/test-integration.yaml -->
| Release (`release.yaml`) | Push to `master`, `beta`, `canary`, `dev`, `*.x` | Runs semantic-release to publish to npm and deploy docs | <!-- source: .github/workflows/release.yaml -->
| CodeQL (`codeql.yaml`) | Push/PR | GitHub security scanning | <!-- source: .github/workflows/codeql.yaml -->

# File-Level Guidance

| Path | Why restricted |
|---|---|
| `dist/` | Generated build output. Never commit or hand-edit. Produced by `npm run build`. |
| `package.json` version field | Controlled by semantic-release. Never manually bump. |
| `lib/adapters/REST/endpoints/` | Must stay 1:1 with `lib/entities/`. Adding an entity requires adding its endpoint file. |
| `lib/create-*-api.ts` | Legacy client factories. Do not add new methods here — use plain client endpoints only. |
