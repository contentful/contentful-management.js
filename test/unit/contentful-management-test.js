...
test('Passes along HTTP client parameters', () => {
  ...
  const createHttpClientStub = jest.fn()
  ...
  expect(createHttpClientStub.mock.calls[0][1].headers['Content-Type']).toBeTruthy()
...
test('Generate the correct User Agent Header', () => {
  ...
  const headerParts = createHttpClientStub.mock.calls[0][1].headers['X-Contentful-User-Agent'].split('; ')
