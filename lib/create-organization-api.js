 ...
 * @prop {function} getTeamMembershipByTeam
 * @prop {function} getTeamMembershipsByTeam
 */
...
export default function createOrganizationApi ({ http }) {
  ...
  const {wrapTeamMembershipByTeam, wrapTeamMembershipsCollectionByTeam} = entities.teamMembership
   ...
   *
   * client.getOrganizations()
   * .then((organization) => organization.getTeamMembershipByTeam('teamId', 'teamMembership_id'))
   * .then((teamMembership) => console.log(teamMembership))
   * .catch(console.error)
   */
  function getTeamMembershipByTeam (teamId, teamMembershipId) {
    return http.get('teams/' + teamId + '/team_memberships/' + teamMembershipId, {headers: teamAlphaHeaders})
      .then(response => wrapTeamMembershipByTeam(http, response.data, 'team1'), errorHandler)
  }
   ...
   *
   * client.getOrganizations()
   * .then((organization) => organization.getTeamMembershipsByTeam('teamId'))
   * .then((teamMemberships) => console.log(teamMemberships))
   * .catch(console.error)
   */
  function getTeamMembershipsByTeam (teamId, query = {}) {
    return http.get('teams/' + teamId + '/team_memberships', {headers: teamAlphaHeaders, query})
      .then(response => wrapTeamMembershipsCollectionByTeam(http, response.data, 'team1'), errorHandler)
  }
