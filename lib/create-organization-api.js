...
export default function createOrganizationApi ({ http }) {
  ...
  function getOrganizationMemberships (query = {}) {
    return http.get('organization_memberships', createRequestConfig({query: query}))
  ...
  function getTeamMemberships (opts = {}) {
    ...
    if (opts.teamId) {
      return http.get('teams/' + opts.teamId + '/team_memberships', createRequestConfig({query: query}))
        .then(response => wrapTeamMembershipCollection(http, response.data), errorHandler)
    }
    return http.get('team_memberships', createRequestConfig({query: query}))
      .then(response => wrapTeamMembershipCollection(http, response.data), errorHandler)
  }
