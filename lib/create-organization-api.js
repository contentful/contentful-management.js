...
export default function createOrganizationApi ({ http }) {
  ...
  const {wrapOrganizationSpaceMembership, wrapOrganizationSpaceMembershipCollection} = entities.organizationSpaceMembership
  ...
  /**
   * Gets an Organization Space Membership
   * @memberof ContentfulOrganizationAPI
   * @param {string} id - Organiztion Space Membership ID
   * @return {Promise<OrganizationSpaceMembership.OrganizationSpaceMembership>} Promise for a Organization Space Membership
   * @example
   * const contentful = require('contentful-management')
   *
   * client.getOrganizations()
  ...
  function getOrganizationSpaceMembership (id) {
    return http.get('space_memberships/' + id, {
      ...
      .then(response => wrapOrganizationSpaceMembership(http, response.data, orgId), errorHandler)
  }

  /**
   * Gets a collection of Organization Space Memberships
   * @memberof ContentfulOrganizationAPI
   * @param {object=} query - Object with search parameters. Check the <a href="https://www.contentful.com/developers/docs/javascript/tutorials/using-js-cda-sdk/#retrieving-entries-with-search-parameters">JS SDK tutorial</a> and the <a href="https://www.contentful.com/developers/docs/references/content-delivery-api/#/reference/search-parameters">REST API reference</a> for more details.
   * @return {Promise<OrganizationSpaceMembership.OrganizationSpaceMembershipCollection>} Promise for a collection of Organization Space Memberships
   * @example
   * const contentful = require('contentful-management')
   *
   * client.getOrganizations()
  ...
  function getOrganizationSpaceMemberships (query = {}) {
    return http.get('space_memberships', {
      ...
      .then((response) => wrapOrganizationSpaceMembershipCollection(http, response.data), errorHandler)
  }
