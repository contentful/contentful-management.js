...
export default function createSpaceApi ({
   ...
   * @memberof ContentfulSpaceAPI
   * @param  {string} id
   * @return {Promise<Upload.Upload>} Promise for an Upload
   * @example
   * space.getUpload('uploadId')
   * .then(upload => console.log(upload))
   */
  function getUpload (id) {
    return httpUploads.get('uploads/' + id)
    .then((response) => wrapEntry(http, response.data), errorHandler)
  }
   ...
   * @memberof ContentfulSpaceAPI
   * @see {Upload.Upload}
   * @param {object} data - File stream representation the files content of the upload to be created. Can be a string, an ArrayBuffer, or a Stream.
   * @return {Promise<Upload.Upload>} Promise for the newly created Upload
   * @example
   * const stream = fs.createReadStream('/path/to/your.file')
   * space.createUpload(stream)
   * .then(upload => console.log(upload))
   */
  function createUpload (data) {
    return httpUploads.post('uploads', data, {
      headers: {
