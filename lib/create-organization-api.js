...
export default function createOrganizationApi ({ http }) {
  ...
  const {wrapTeamMembership, wrapTeamMembershipCollection, wrapOrganizationTeamMembershipCollection} = entities.teamMembership
  ...
  function createTeamMembership (teamId, data = {}) {
    return http.post('teams/' + teamId + '/team_memberships', data, {headers: teamAlphaHeaders})
  ...
  /**
   * Get all Team Memberships for the team
   * @memberof ContentfulOrganizationAPI
   * @param {string} id - Team Membership ID
  ...
  /**
   * Gets all Team Memberships of the organization
   * @memberof ContentfulOrganizationAPI
   * @return {Promise<TeamMembership[]>} Promise for a Team Membership Collection
   * @example
   * const contentful = require('contentful-management')
   *
   * client.getOrganization('organizationId')
   * .then((organization) => organization.getOrganizationTeamMemberships())
   * .then((teamMemberships) => console.log(teamMemberships))
   * .catch(console.error)
   */
  function getOrganizationTeamMemberships (query = {}) {
    return http.get('team_memberships', {headers: teamAlphaHeaders, query})
      .then(response => wrapOrganizationTeamMembershipCollection(http, response.data), errorHandler)
  }
  ...
  return {
    ...
    getTeamMembership,
    getTeamMemberships,
    getOrganizationTeamMemberships,
