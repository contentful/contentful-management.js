...
import { entryTagTests, assetTagTests } from './entity-tag-integration'
...
test('Gets spaces', (t) => {
  t.plan(2)
  return client.getSpaces().then((response) => {
...
test('Gets organizations', (t) => {
  t.plan(1)
  return client.getOrganizations().then((response) => {
...
test('Get organization usage', (t) => {
  t.plan(5)
  return v2Client
    .getOrganizationUsage(v2Organization, {
...
test('Fails getting org usage for invalid dateRange', (t) => {
  t.plan(1)
  t.shouldFail(
    v2Client.getOrganizationUsage(v2Organization, {
      'dateRange.startAt': '2020-01-01',
...
test('Fails getting space usage for invalid dateRange', (t) => {
  t.plan(1)
  t.shouldFail(
    v2Client.getSpaceUsage(v2Organization, {
      'dateRange.startAt': '2020-01-01',
      'dateRange.endAt': '2019-01-01',
    })
  )
...
test('Gets personal access tokens', (t) => {
  t.plan(2)
  return client.getPersonalAccessTokens().then((response) => {
    t.ok(response.items, 'items')
...
test('Create and revoke a personal access token', (t) => {
  ...
  return client
    ...
    .then((token) => {
      ...
      return token.revoke().then((revoked) => {
...
test('Gets space', (t) => {
  t.plan(2)
  return client.getSpace('ezs1swce23xe').then((response) => {
    t.ok(response.sys, 'sys')
...
test('Fails to get space', (t) => {
  t.plan(2)
  return client.getSpace(generateRandomId('weirdrandomid')).then(
    () => {},
    (error) => {
...
test('Creates, updates and deletes a space', (t) => {
  ...
  return client
    ...
    .then((space) => {
      ...
      return space.update().then((updatedSpace) => {
...
test('gets V2 space for entity tag tests', (t) => {
  return v2Client.getSpace('w6xueg32zr68').then((space) => {
    entryTagTests(t, space)
    assetTagTests(t, space)
  })
})
...
test('Create space for tests of space membership', (t) => {
  return client.getOrganizations().then((response) => {
    const organization = response.items[0]
    client.getSpace('ezs1swce23xe').then((space) => {
      return Promise.all([spaceMembershipTests(t, organization, space)])
    })
  })
})
...
test('Logs request and response with custom loggers', (t) => {
  ...
  return client.getSpace('ezs1swce23xe').then(() => {
    ...
    t.equal(
      `${baseURL}${url}`,
      'https://api.contentful.com:443/spaces/ezs1swce23xe',
      'requestLogger is called with correct url'
    )
  })
})
