...
export default function createOrganizationApi ({ http }) {
  ...
  const {wrapInvitation} = entities.invitation
   ...
   * @return {Promise<Invitation>} Promise for a Invitation in an organization
   * @example
   * const contentful = require('contentful-management')
   *
   * client.getOrganizations()
   * .then((organization) => organization.getInvitation('invitation_id'))
  ...
  function getInvitation (id) {
    return http.get('invitations/' + id, {
      ...
      .then(response => wrapInvitation(http, response.data), errorHandler)
  }
   ...
   *
   * client.getOrganizations()
   *  .then((organization) => organization.createInvitation({
