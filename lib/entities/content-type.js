...
function createContentTypeApi (http) {
  return {
    ...
    omitAndDeleteField: function (id) {
      const changedField = this.changeField(id, 'omitted', true)
      if (!changedField) {
        return Promise.reject(new Error(`Tried to omitAndDeleteField on a nonexistent field on the content type ${this.name}.`))
      }
      return this.update().then(newContentType => {
        newContentType.changeField(id, 'deleted', true)
        return newContentType.update()
      })
        .catch(errorHandler)
    },
     ...
     * @return {null}
     */
    changeField: function (id, key, value) {
