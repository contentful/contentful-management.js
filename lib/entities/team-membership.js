...
function createTeamMembershipApi (http, teamId) {
  return {
     ...
     * .then(org => org.getTeamMembershipByTeam('teamId', 'teamMembership_id'))
     * .then((teamMembership) => {
 ...
 */
export function wrapTeamMembershipByTeam (http, data, teamId) {
 ...
 */
export function wrapTeamMembershipCollectionByTeam (http, data, teamId) {
  const teamMemberships = toPlainObject(cloneDeep(data))
  teamMemberships.items = teamMemberships.items.map((entity) => wrapTeamMembershipByTeam(http, entity, teamId))
